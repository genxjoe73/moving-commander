...
export async function QuotesWorkspace() {
  ...
  return <><header className="app-header"><div><p className="eyebrow">ESTIMATING</p><h1>Quotes</h1><p>Draft estimates created from qualified opportunities. Rate calculation and tariff rules are the next quote slice.</p></div></header><section className="workspace-panel"><div className="section-heading"><p className="eyebrow">QUOTE REGISTER</p><h2>{records.length} quotes</h2></div>{records.length === 0 ? <div className="inline-empty"><h3>No quotes yet</h3><p>Open Leads and convert a qualified opportunity.</p></div> : <div className="data-table-wrap"><table className="data-table"><thead><tr><th>Quote</th><th>Customer</th><th>Status</th><th>Move date</th><th>Estimate</th></tr></thead><tbody>{records.map((item) => <tr key={item.id}><td><strong>{item.quoteNumber}</strong></td><td>{item.companyName || [item.firstName, item.lastName].filter(Boolean).join(" ") || "—"}</td><td><span className={`status-pill status-${item.status}`}>{item.status}</span></td><td>{item.moveDate ? new Intl.DateTimeFormat("en-US", { dateStyle: "medium" }).format(item.moveDate) : "TBD"}</td><td>{new Intl.NumberFormat("en-US", { style: "currency", currency: "USD" }).format(Number(item.estimatedTotal))}</td></tr>)}</tbody></table></div>}</section></>;
}
